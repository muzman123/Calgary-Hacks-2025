...
# Import our simulated sensor functions
from sensor import is_area_active, get_current_spl
...
CMAP = 'magma'            # Colormap
SENSOR_THRESHOLD = 57.0   # dB threshold for activity
...
def record_audio(duration=RECORD_SECONDS, sample_rate=SAMPLE_RATE):
    ...
    """
    print(f"Recording {duration} seconds of audio from mic...")
...
def index():
    # Render the base page that will start polling once the user clicks "Start"
    return render_template('index.html')

@app.route('/predict', methods=['GET'])
def predict():
    """
    This endpoint is polled by the browser every 5 seconds.
    It first checks the simulated sensor:
      - If sensor value is above SENSOR_THRESHOLD, it records and predicts.
      - Otherwise, returns a JSON message indicating the area is calm.
    It also returns the current simulated SPL value.
    """
    current_spl = get_current_spl()
    if is_area_active(threshold_db=SENSOR_THRESHOLD):
        predicted_label, filename = predict_from_audio()
        image_url = url_for('static', filename=f'spectrograms/{filename}', _external=True)
        response = {
            "active": True,
...
def sensor_endpoint():
    """
    Returns the current SPL value from the simulated sensor.
    """
    current_spl = get_current_spl()
    return jsonify({"current_spl": current_spl})
